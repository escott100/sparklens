...
class EfficiencyStatisticsAnalyzer extends  AppAnalyzer {
  ...
  def produceMetrics(appContext: AppContext, startTime: Long, endTime: Long): mutable.Map[String, Double] = {
    val ac = appContext.filterByStartAndEndTime(startTime, endTime)
    val appTotalTime = endTime - startTime
    val jobTime = JobOverlapHelper.estimatedTimeSpentInJobs(ac)
    val maxExecutors = AppContext.getMaxConcurrent(ac.executorMap, ac)
    ...
    val metrics = mutable.Map[String,Double]()
    metrics += ("time_spent_in_driver" -> ((driverTimeJobBased + criticalPathTime)/(60*1000)), "time_spent_in_executor" -> (jobTime)/1000)
    metrics += ("minimum_app_time_critical_path" -> ((driverTimeJobBased + criticalPathTime)/(60*1000)), "minimum_time_with_same_executors" -> ((driverTimeJobBased + perfectJobTime)/(60*1000)))
    metrics += ("oneCoreComputeHours_mins" -> (appComputeMillisAvailable/(60*1000)), "oneCoreComputeHours_wasted_by_driver_mins" -> (driverComputeMillisWastedJobBased/(60*1000)))
    metrics += ("executor_oneCoreComputeHours_available_mins" -> (inJobComputeMillisAvailable/(60*1000)), "executor_oneCoreComputeHours_used_mins" -> (inJobComputeMillisUsed/(60*1000)))
    return metrics
  }
}
