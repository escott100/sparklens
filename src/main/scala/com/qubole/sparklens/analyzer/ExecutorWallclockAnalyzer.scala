      ...
      pool.execute( new Runnable {
        override def run(): Unit = {
          ...
          if (executorCount > 0) {
            ...
            results.synchronized {
              results(percent) = f" Executor count ${executorCount}%5s  ($percent%3s%%) estimated time ${pd(estimatedTime)} and estimated cluster utilization ${utilization}%3.2f%%"
              metrics += ("Time_With_" + percent.toString() + "percent_Executors" -> (estimatedTime.toDouble)/1000, "Cluster_Utilization_with_" + percent.toString() + "percent_Executors" -> utilization.toDouble)
            }
          }
        }
      })
